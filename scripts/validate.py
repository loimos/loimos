...
def parse_args():
    ...
    parser.add_argument('-np', '--num-partitions', default=128, type=int,
...
def main():
    ...
    if args.state is not None:
        state = args.state
        activity_location_assignment = pd.read_csv(os.path.join(input_dir,
            f'{state}_activity_location_assignment_week_final.csv'))
        activity_locations = pd.read_csv(os.path.join(input_dir,
            f'{state}_activity_locations_final.csv'))
        residence_locations = pd.read_csv(os.path.join(input_dir,
            f'{state}_residence_locations_final.csv'))
        ...
        print('  Activity visits lid range: {} to {}' \
                .format(activity_visits['lid'].min(),
                    activity_visits['lid'].max()))
        print('  Activity locations lid range: {} to {}' \
                .format(activity_locations['lid'].min(),
                    activity_locations['lid'].max()))
        print('  Residence visits lid range: {} to {}' \
                .format(residence_visits['lid'].min(),
                    residence_visits['lid'].max()))
        print('  Residence locations lid range: {} to {}' \
                .format(residence_locations['lid'].min(),
                    residence_locations['lid'].max()))
    ...
    print('  Visits pid range: {} to {}' \
            .format(visits['pid'].min(), visits['pid'].max()))
    print('  People pid range: {} to {}' \
            .format(people['pid'].min(), people['pid'].max()))
    print('  Visits lid range: {} to {}' \
            .format(visits['lid'].min(), visits['lid'].max()))
    print('  Locations lid range: {} to {}' \
            .format(locations['lid'].min(), locations['lid'].max()))
