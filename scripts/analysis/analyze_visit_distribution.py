...
def parse_args():
    ...
    parser.add_argument('output_dir', metavar='O',
...
def analyze_by_location(locations, people, visits, output_dir):
    ...
    print(visit_counts_by_location['num_visit_sources'].mean(),
...
def analyze_by_person(locations, people, visits, output_dir):
    ...
    fig, ax = plt.subplots(figsize=(10,6))
    sns.histplot(visited_person_counts, bins=50, kde=False,
    ...
    visit_counts_by_person = visits_by_person.count()\
            .reset_index()
    ...
    fig, ax = plt.subplots(figsize=(10,6))
    sns.histplot(visit_counts_by_person, bins=50, x='pid', y='count',
            kde=False)
    plt.savefig(os.path.join(output_dir, 'visit_sources_hist.pdf'))

    print(visit_counts_by_person['start_time'].mean(),
