...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Scripts to perform pre-run load balancing."
    )
    parser.add_argument(
        "location_method",
        choices=LOCATION_SCHEMES,
        help="Location partitioning scheme to use",
    )
    parser.add_argument(
        "people_method", choices=PEOPLE_SCHEMES, help="People partitioning scheme to us"
    )
    ...
    if args.location_method == "VISIT":
        location_clusters = lp.graph_partioning_clustering(
            locations, int(args.num_location_partitions), people_location_visit_graph
        )
    else:
        location_clusters = lp.geo_clustering(
            locations, 4, int(args.num_location_partitions)
        )

    # Create people clusters.
    people_clusters = pp.greedy_affinity_to_clusters(
        location_clusters, int(args.num_people_partitions), people_location_visit_graph
    )
    ...
    people, locations, visits = ids.remap(
        people.reindex(people_remapped_ids),
        locations.reindex(people_remapped_ids),
        visits,
        people_remapped_ids,
        location_remapped_ids,
    )
