...
from utils.ids import remap, init_multiprocessing  # noqa
...
def get_partition_mean(
    ...
    df[partition_col] = pd.cut(df[partition_by], num_partitions).cat.codes
...
def main():
    ...
    non_home_partitions, _ = folding_partition(
        num_non_homes - num_extra, num_partitions, i_0=num_homes + num_extra
    )
    for i in range(num_partitions):
    ...
    people, locations, visits = remap(
        people,
        locations,
        visits,
        new_location_ids=inverted_permutation,
        num_partitions=num_merge_partitions,
        num_tasks=num_tasks,
    )
