...
People::People() {
	...
	numLocalPeople = getNumLocalElems(numPeople, numPeopleSubsets, thisIndex);
...
void People::SendVisitMessages() {
	...
	for(int i=0; i<numLocalPeople; i++) {
		personIdx = getGlobalIndex(i,thisIndex,numPeople,numPeopleSubsets);    
		...
		for(int j=0; j<visits; j++) {
			...
			locationSubset = getContainerIndex(locationIdx,numLocations,numLocationSubsets);
...
void People::UpdateDiseaseState(int personIdx, char state) {
	// updating state of a person
	int localIdx = getLocalIndex(personIdx,numPeople,numPeopleSubsets);
	peopleState[localIdx] = state;
	//CkPrintf("Person %d state %d\n",personIdx,state);
}

void People::ReportStats() {
	int total = 0;
