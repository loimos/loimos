...
class Locations : public CBase_Locations {
 private:
  ...
  std::unordered_map<PartitionId, std::vector<InteractionMessage>> createPartitionToMessagesMapping(Location *loc, std::unordered_map<Id, std::vector<Interaction>> *interactions);
